...
- Transition Function (T): The rules that tell us how we move from one state to another when an action is taken, represented as \( p(s'|s, a) \). This is the probability of transitioning to state \( s' \) from state \( s \) after taking action \( a \).
